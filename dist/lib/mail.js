...
class Mail {
  ...
  validateConfig() {
    ...
    if (ValidationService.isObject(this.transporterOptions)) {
      if (!Object.keys(this.transporterOptions).length || Object.values(this.transporterOptions).every(value => ValidationService.isNullOrUndefinedOrEmpty(value))) {
        this.isConfigured = false;
        return true;
      }
      if (!ValidationService.validateBody(this.transporterOptions, {
        host: value => ValidationService.validateString({
          value,
          min: 4,
          noWhiteSpace: true
        }),
        ...
        auth: value => ValidationService.isNullOrUndefinedOrEmpty(value) || ValidationService.isObject(value) && ValidationService.validateString({
          value: value.user,
          min: 1
