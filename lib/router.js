...
class Router {
  ...
  static toPrefixedRoute(prefix, route) {
    ...
    if (formattedPrefix.endsWith("/") && formattedPrefix !== "/") {
      formattedPrefix = formattedPrefix.substring(
        0,
        formattedPrefix.length - 1
      );
    }
    ...
    return `${formattedPrefix === "/" ? "" : formattedPrefix}${
      formattedRoute.startsWith("/") ? formattedRoute : `/${formattedRoute}`
    }`;
  }
  static init({
    prefix,
    ...
    HttpAdapter.get(
      Router.toPrefixedRoute(prefix, "/"),
      Controllers.getMeController
    );
    HttpAdapter.post(
      Router.toPrefixedRoute(prefix, "/login"),
      Controllers.loginController
    );
    HttpAdapter.post(
      Router.toPrefixedRoute(prefix, "/register"),
      Controllers.registerController
    );
    HttpAdapter.post(
      Router.toPrefixedRoute(prefix, "/refresh-token"),
      Controllers.refreshTokenController
    );
    HttpAdapter.post(
      Router.toPrefixedRoute(prefix, "/forgot-password"),
      Controllers.sendForgotPasswordRequestController
    );
    HttpAdapter.post(
      Router.toPrefixedRoute(prefix, "/forgot-password/:id"),
      Controllers.forgotPasswordController
    );
    HttpAdapter.post(
      Router.toPrefixedRoute(prefix, "/change-password"),
      Controllers.updatePasswordController
    );
    HttpAdapter.get(
      Router.toPrefixedRoute(prefix, "/verify/:id"),
      Controllers.verifyAccountController
    );
    HttpAdapter.post(
      Router.toPrefixedRoute(prefix, "/verify"),
